...
Tidying 
This code cleans and transforms the movie_industry dataset by extracting the release date, splitting runtime into hours and minutes, and creating a movie_duration column. It standardizes movie ratings (e.g., "X" to "NC-17") and calculates profit by subtracting budget from gross revenue. Unnecessary columns are removed, and movies with "Not Rated" or "Unrated" ratings are filtered out. The genre and rating columns are converted to factors, and column names are renamed for clarity. These steps prepare the data for analysis by ensuring consistency, creating useful variables, and organizing the dataset.
```{r}
